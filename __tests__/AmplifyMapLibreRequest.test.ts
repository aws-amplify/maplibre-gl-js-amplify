...
describe('AmplifyMapLibreRequest', () => {
  ...
  test('transformRequest returns undefined for non amazon and malicious urls', () => {
    ...
    const amplifyRequest = new AmplifyMapLibreRequest(mockCreds, 'us-west-2');
    expect(
      amplifyRequest.transformRequest(
        'http://maps.geo.evil-amazonaws.com/?x=amazonaws.com',
        'any'
      )
    ).toBe(undefined);
    expect(
      amplifyRequest.transformRequest(
        'http://amazonaws.com.evil-amazonaws.com',
        'any'
      )
    ).toBe(undefined);
    const request = amplifyRequest.transformRequest(
      'http://maps.geo.us-east-1.amazonaws.com',
      'any'
    );
  ...
  test('transformRequest appends query params to existing query params if any', () => {
    ...
    let request = amplifyRequest.transformRequest(
      'http://maps.geo.us-east-1.amazonaws.com?tsi=0',
      'any'
    );
    const queryStringStartIndex = request.url.indexOf('?');
    const anotherQueryStringStartIndexExists =
      request.url.indexOf('?', queryStringStartIndex + 1) !== -1;
    ...
    request = amplifyRequest.transformRequest(
      'http://maps.geo.us-east-1.amazonaws.com',
      'any'
    );
    ...
    request = amplifyRequest.transformRequest(
      'http://maps.geo.us-east-1.amazonaws.com?param1=1&',
      'any'
    );
    searchParams = new URL(request.url).searchParams;
    ...
    request = amplifyRequest.transformRequest(
      'http://maps.geo.us-east-1.amazonaws.com?param1=1&param2=2',
      'any'
    );
    searchParams = new URL(request.url).searchParams;
    expect(searchParams.has('param1')).toBe(true);
